...
public class EnvironmentRoutes extends AllDirectives {
    ...
    public EnvironmentRoutes(ActorRef<EnvironmentCoordinator.Command> environmentCoordinator, ActorRef<AirCondition.AirConditionCommand> airConditionActor) {
    ...
    private Route setFixedTemperature() {
        return parameter("value", valueStr -> {
            try {
                ...
                return complete(StatusCodes.BAD_REQUEST, new ErrorResponse("Invalid temperature value"), Jackson.marshaller());
            }
        });
    }
    ...
    private Route setFixedWeather() {
        return parameter("condition", conditionStr -> {
            try {
                ...
                return complete(StatusCodes.BAD_REQUEST, new ErrorResponse(ex.getMessage()), Jackson.marshaller());
            }
        });
    }
